...
Модульная модель также отражена в таблице `modules` миграции PostgreSQL.
...
В PostgreSQL это отражено таблицей `headless_settings`.
...
- `install.php` — веб-установщик, который собирает название, описание и URL сайта, проверяет PostgreSQL-подключение, выполняет миграцию `database/migrations/0001_create_initial_schema.sql`, создает администратора и сохраняет настройки в `.env`.
...
Затем измените параметры `APP_NAME`, `APP_URL`, `DB_HOST`, `DB_PORT`, `DB_DATABASE`, `DB_USERNAME` и `DB_PASSWORD` под свое окружение PostgreSQL.
...
1. Установите Node.js 20+, PHP с расширением `pdo_pgsql` и PostgreSQL.
2. Подготовьте пустую PostgreSQL-базу и пользователя, который может создавать таблицы, типы и расширение `pgcrypto`.
...
4. Откройте `http://localhost:8080/install.php` и пройдите шаги установки: параметры сайта, данные PostgreSQL, данные администратора и подтверждение запуска миграции.
